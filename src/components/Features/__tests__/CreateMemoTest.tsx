...
jest.mock('@/actions/memo.actions', () => ({
  postMemo(input: string, textarea: string, email: string) {
    return mockPostMemo(input, textarea, email);
  },
}));
...
describe('메모 등록 기능 테스트', () => {
  ...
  test('입력창을 모두 입력하지 않으면 메모가 등록되지 않고 예외가 발생한다.', async () => {
    ...
    await waitFor(
      () => {
        expect(mockPostMemo).toHaveBeenCalledTimes(0);
  ...
  test('사용자 정보에 이메일이 있고, 입력창이 모두 비어있지 않으면 정상적으로 메모가 등록 된다.', async () => {
    ...
    await waitFor(
      () => {
        expect(mockPostMemo).toHaveBeenCalledWith(
          TITLE_INPUT,
          CONTENT_INPUT,
          SESSION.user.email,
        );
