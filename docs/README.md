...
- [x] 등록 기능
  - [ ] 등록하기 버튼을 누르면 등록 페이지로 이동한다. `my/memo/create`
  - [x] 입력 기능
    - [x] 제목을 입력 받는다.
    - [x] 내용을 입력 받는다.
  - [x] 버튼을 누르면 입력한 메모가 DB에 등록된다.
  - [x] 정상적으로 등록이 되면 메시지를 보여준다.
  - [ ] 목록으로 가기 버튼을 누르면 My 페이지로 돌아간다.
- [ ] My 페이지에서 등록된 메모를 테이블로 보여준다.
  - [ ] 데이터베이스에서 가져온다.
  - [ ] 메모 제목을 누르면 해당 메모 페이지로 이동한다.
  - [ ] `id` 값을 가지고 해당 메모 페이지 `my/memo/idr값`에서 자세히 볼 수 있다.
